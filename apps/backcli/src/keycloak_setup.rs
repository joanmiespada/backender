...
impl KeycloakSetup {
    pub fn from_env() -> Result<Self, String> {
        ...
        let admin_user = std::env::var("KEYCLOAK_ADMIN_USER")
    ...
    async fn get_client_secret(&self, token: &str, client_uuid: &str) -> Result<String, String> {
        ...
        if !response.status().is_success() {
            return Err(format!("Failed to retrieve client secret: {}", response.status()));
        }
    ...
    async fn get_realm_management_client_id(&self, token: &str) -> Result<String, String> {
        ...
        let response = self.http
            ...
            .send()
            .await
            .map_err(|e| format!("Failed to get {} client: {}", client_id, e))?;

        if !response.status().is_success() {
            return Err(format!("Failed to query {} client: {}", client_id, response.status()));
        }

        let clients: Vec<ClientDetails> = response
            .json()
            .await
            .map_err(|e| format!("Failed to parse {} response: {}", client_id, e))?;
    ...
    async fn get_service_account_user_id(&self, token: &str, client_uuid: &str) -> Result<String, String> {
        let url = format!(
        ...
        if !response.status().is_success() {
            return Err(format!("Failed to retrieve service account user: {}", response.status()));
        }
    ...
    async fn get_client_roles(&self, token: &str, realm_mgmt_uuid: &str) -> Result<Vec<(String, String)>, String> {
        let url = format!(
        ...
        if !response.status().is_success() {
            return Err(format!("Failed to retrieve client roles: {}", response.status()));
        }
    ...
    async fn assign_client_roles(&self, token: &str, user_id: &str, realm_mgmt_uuid: &str, role_ids: Vec<(String, String)>) -> Result<(), String> {
        let url = format!(
    ...
    pub async fn setup_service_account(&self, client_id: &str) -> Result<String, String> {
        ...
        println!("Checking if client '{}' exists...", client_id);
        ...
        let sa_user_id = self.get_service_account_user_id(&token, &client_uuid).await?;
        ...
        self.assign_client_roles(&token, &sa_user_id, &realm_mgmt_uuid, required_roles).await?;
