...
use user_lib::util::*;
use user_lib::{
    repository::{RoleRepository, UserRepository, UserRoleRepository},
...
async fn integration_user_service_flow() {
    ...
    let pool = connect_with_retry(&db_url, 10)
        .await
    ...
    let seeded_roles = user_service
        .get_roles(PaginationParams::default())
        .await
        .unwrap();
    assert_eq!(
        seeded_roles.items.len(),
        2,
        "Should have seeded admin and user roles"
    );
    assert!(
        seeded_roles.items.iter().any(|r| r.name == "admin"),
        "Should have admin role"
    );
    assert!(
        seeded_roles.items.iter().any(|r| r.name == "user"),
        "Should have user role"
    );

    let seeded_users = user_service
        .get_users(PaginationParams::default())
        .await
        .unwrap();
    ...
    assert!(
        seeded_users
            .items
            .iter()
            .any(|u| u.keycloak_id == "root-placeholder-keycloak-id"),
        "Should have root user with placeholder keycloak_id"
    );
    ...
    user_service
        .assign_role(user1.id, role_editor.id)
        .await
        .unwrap();
    user_service
        .assign_role(user2.id, role_editor.id)
        .await
        .unwrap();

    // Should have 4 users total (1 seeded root + 3 created)
    let list_users = user_service
        .get_users(PaginationParams::default())
        .await
        .unwrap();
