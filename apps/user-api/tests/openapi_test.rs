...
fn test_openapi_spec_has_all_endpoints() {
    ...
    assert!(
        paths.contains_key("/roles/{id}"),
        "Missing /roles/{{id}} path"
    );

    // User-role assignment endpoints
    assert!(
        paths.contains_key("/users/{user_id}/roles/{role_id}"),
        "Missing user-role assignment path"
    );
    ...
    assert!(
        user_by_id_path.delete.is_some(),
    ...
    assert!(
        role_by_id_path.delete.is_some(),
        "Missing DELETE /roles/{{id}}"
    );

    // Verify user-role assignment methods
    let user_roles_path = paths.get("/users/{user_id}/roles/{role_id}").unwrap();
    assert!(
        user_roles_path.post.is_some(),
        "Missing POST /users/{{user_id}}/roles/{{role_id}}"
    );
    assert!(
        user_roles_path.delete.is_some(),
        "Missing DELETE /users/{{user_id}}/roles/{{role_id}}"
    );
    ...
    assert!(
        ...
        "Missing UpdateUserRequest schema"
    );
    assert!(
    ...
    assert!(
        schemas.contains_key("CreateRoleRequest"),
        "Missing CreateRoleRequest schema"
    );
    assert!(
        schemas.contains_key("UpdateRoleRequest"),
        "Missing UpdateRoleRequest schema"
    );
    assert!(
        schemas.contains_key("RoleResponse"),
        "Missing RoleResponse schema"
    );
